...
#   - enzh  (binary)    : data/lr_exp/{N}_config/     languages=en,zh   for N in 100 500 1000 10000
#
...
for entry in "${MODELS[@]}"; do
  ...
  if [[ "$SKIP_ENZH" != "1" ]]; then
    for n in $LR_CONFIGS; do
      JOBS+=("$model_hf|$short|enzh-$n|$LR_EXP_DIR/${n}_config|$LANGS_ENZH|$OUT_BASE/cld-$short-enzh-$n")
    done
  fi
done
